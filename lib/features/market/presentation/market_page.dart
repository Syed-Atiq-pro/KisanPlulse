...
class _MarketPageState extends State<MarketPage> {
  ...
  Future<void> _loadMarkets() async {
    try {
      ...
      if (mounted) setState(() => loadingMarkets = false);
    }
  }
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(labelText: 'Target ₹ / quintal'),
              ),
              const SizedBox(height: 12),
              DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(value: 'above', child: Text('Price reaches above target')),
                  DropdownMenuItem(value: 'below', child: Text('Price falls below target')),
                ],
                onChanged: (value) => setDialog(() => direction = value ?? 'above'),
              ),
            ],
          ),
          actions: [
            ...
            FilledButton(
              onPressed: () => Navigator.pop(context, double.tryParse(target.text) != null),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Market Intelligence', style: TextStyle(fontWeight: FontWeight.w800)),
        actions: [
          IconButton(onPressed: _addAlert, icon: const Icon(Icons.notifications_active_rounded), tooltip: 'Price alert'),
        ],
      ),
      body: ListView(
        ...
        children: [
          Card(child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            DropdownButtonFormField<String?>(
              ...
              items: [
                ...
                ...markets.map((m) => DropdownMenuItem<String?>(value: m.id, child: Text('${m.name}${m.district == null ? '' : ' • ${m.district}'}', overflow: TextOverflow.ellipsis))),
              ],
              onChanged: (value) { setState(() => selectedMarket = value); _loadPrices(); },
            ),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            TextButton.icon(onPressed: () => context.push('/market/analytics?commodity=${Uri.encodeComponent(commodity.text.trim())}'), icon: const Icon(Icons.analytics_outlined), label: const Text('Analytics')),
          ]),
          ...
          FutureBuilder<List<MarketPrice>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) return const Card(child: Padding(padding: EdgeInsets.all(20), child: Text('Market data is unavailable. Sync official market data first.')));
              ...
              return Column(
                children: items.map((p) => Card(child: ListTile(
                  leading: const CircleAvatar(child: Icon(Icons.storefront_rounded)),
                  title: Text('${p.commodity} • ₹${p.modalPrice.toStringAsFixed(0)} / ${p.unit}'),
                  subtitle: Text('${p.priceDate.day}/${p.priceDate.month}/${p.priceDate.year} • Min ₹${p.minPrice?.toStringAsFixed(0) ?? '-'} • Max ₹${p.maxPrice?.toStringAsFixed(0) ?? '-'}'),
                  trailing: p.arrivalsTonnes == null ? null : Text('${p.arrivalsTonnes!.toStringAsFixed(1)} t'),
                ))).toList(),
              );
            },
          ),
