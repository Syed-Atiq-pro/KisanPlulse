...
class WeatherSnapshot {
  const WeatherSnapshot({required this.temperature, required this.apparentTemperature, required this.humidity, required this.windSpeed, required this.weatherCode, required this.time});
  ...
  factory WeatherSnapshot.fromJson(Map<String, dynamic> json) {
    ...
    return WeatherSnapshot(temperature: (current['temperature_2m'] as num).toDouble(), apparentTemperature: (current['apparent_temperature'] as num).toDouble(), humidity: (current['relative_humidity_2m'] as num).toDouble(), windSpeed: (current['wind_speed_10m'] as num).toDouble(), weatherCode: (current['weather_code'] as num).toInt(), time: DateTime.parse(current['time'] as String));
  }
}

class DailyForecast {
  const DailyForecast({required this.date, required this.max, required this.min, required this.precipitation, required this.code});
  final DateTime date;
  final double max;
  final double min;
...
class WeatherService {
  Future<WeatherSnapshot> current(double latitude, double longitude) async {
    final uri = Uri.https('api.open-meteo.com', '/v1/forecast', {'latitude': '$latitude', 'longitude': '$longitude', 'current': 'temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,weather_code', 'timezone': 'auto'});
  ...
  Future<List<DailyForecast>> daily(double latitude, double longitude) async {
    final uri = Uri.https('api.open-meteo.com', '/v1/forecast', {'latitude': '$latitude', 'longitude': '$longitude', 'daily': 'weather_code,temperature_2m_max,temperature_2m_min,precipitation_sum', 'forecast_days': '7', 'timezone': 'auto'});
    final response = await http.get(uri).timeout(const Duration(seconds: 12));
    if (response.statusCode != 200) throw Exception('Weather service returned ${response.statusCode}.');
    ...
    return List.generate(dates.length, (i) => DailyForecast(date: DateTime.parse(dates[i]), max: max[i].toDouble(), min: min[i].toDouble(), precipitation: rain[i].toDouble(), code: codes[i].toInt()));
  }
}
