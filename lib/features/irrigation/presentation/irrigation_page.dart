...
class _IrrigationPageState extends State<IrrigationPage> {
  ...
  Future<void> _loadWeather() async {
    ...
    try {
      ...
      if (mounted) setState(() => error = 'Could not load weather-based water demand: $e');
    } finally {
  ...
  void _calculate() {
    ...
    setState(() {
      error = null;
      result = calculator.calculate(et0Mm: et0, cropCoefficient: kc, rainMm: rain);
    });
  }

  @override
  void dispose() {
    et0Controller.dispose();
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const Card(child: Padding(padding: EdgeInsets.all(18), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.info_outline), SizedBox(width: 12), Expanded(child: Text('AgriSense uses the FAO-style ET₀ × Kc approach as a starting point. Soil type, effective rainfall, irrigation efficiency and measured soil moisture should be incorporated before automating irrigation.'))]))),
        ],
      ),
    );
  }
...
class _RecommendationCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(margin: const EdgeInsets.only(top: 16), child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 18),
      LinearProgressIndicator(value: (result.netWaterMm / 12).clamp(0.0, 1.0).toDouble()),
...
class _Metric extends StatelessWidget {
  ...
  final String value;
  @override
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(value, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800)), const SizedBox(height: 4), Text(label, style: Theme.of(context).textTheme.bodySmall)]);
}
