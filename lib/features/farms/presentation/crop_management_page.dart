...
class CropManagementPage extends ConsumerWidget {
  ...
  Future<void> _addCrop(BuildContext context, WidgetRef ref) async {
    ...
    final result = await showDialog<bool>(context: context, builder: (dialogContext) => StatefulBuilder(builder: (context, setDialogState) => AlertDialog(
      ...
      actions: [TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Cancel')), FilledButton(onPressed: () async { if (name.text.trim().isEmpty) return; await CropRepository(Supabase.instance.client).createCrop(fieldId: fieldId, name: name.text, variety: variety.text, status: status, stage: stage, plantingDate: planting, expectedHarvestDate: harvest, notes: notes.text, expectedYieldKg: double.tryParse(yieldController.text)); if (dialogContext.mounted) Navigator.pop(dialogContext, true); }, child: const Text('Add crop'))],
    )));
