...
final assistantRepositoryProvider = Provider<AssistantRepository>((ref) => AssistantRepository(Supabase.instance.client));
...
class _AssistantPageState extends ConsumerState<AssistantPage> {
  ...
  Widget build(BuildContext context) {
    const suggestions = ['What should I do for my crops today?', 'How much water does my crop need?', 'How can I improve my soil?', 'Check my recent disease risks'];
    return Scaffold(
      appBar: AppBar(
        title: const Text('AI Farming Assistant', style: TextStyle(fontWeight: FontWeight.w800)),
        actions: [
          IconButton(onPressed: _startConversation, tooltip: 'New chat', icon: const Icon(Icons.add_comment_rounded)),
                ...
                ? ListView(
                    ...
                    children: [
                      Card(child: Padding(padding: const EdgeInsets.all(22), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        const CircleAvatar(radius: 28, child: Icon(Icons.auto_awesome_rounded)),
                        const SizedBox(height: 16),
                        Text('Your farm-aware AI assistant', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w800)),
                        const SizedBox(height: 8),
                        const Text('Ask about crops, soil, irrigation, weather, disease symptoms, or your next farm task. The assistant uses your AgriSense farm data when available.'),
                        const SizedBox(height: 18),
                        ...suggestions.map((s) => Padding(padding: const EdgeInsets.only(bottom: 8), child: OutlinedButton.icon(onPressed: () => _send(s), icon: const Icon(Icons.arrow_outward_rounded, size: 18), label: Align(alignment: Alignment.centerLeft, child: Text(s))))),
                      ])),
                    ],
                  )
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      if (loading && index == messages.length) return const Padding(padding: EdgeInsets.all(16), child: Align(alignment: Alignment.centerLeft, child: CircularProgressIndicator()));
